%% SRM Simulation: KEMAR Database (Per-Target Breakdown)
% Computes Spatial Release from Masking (SRM) using Jelfs2011 model
% Shows individual target positions (-30°, 0°, +30°) vs head-locked mean
%
% Setup:
% - Database: KEMAR HRTF
% - Targets: -30°, 0°, +30° 
...
%
% Outputs:
% - Polar plot: Per-target SRM + head-locked mean
% - Line plot: Per-target SRM + head-locked mean
%
...
%for world-fixed sources
for j = 1:length(tgt_angles)
    polarplot(pax, angles_rad, mean_snr_fixed(j,:),'DisplayName', sprintf('Fixed World with target at %d°', tgt_angles(j)), 'Color', colors(j,:),'LineWidth', 1.2);
end
...
%varying with head angle
for tgt_cnt = 1:length(tgt_angles)
    plot(head_angles, mean_snr_fixed(tgt_cnt,:), '-o','DisplayName', sprintf('Fixed, Target %d°', tgt_angles(tgt_cnt)),'Color', colors(tgt_cnt,:), 'LineWidth', 1.2);
end
